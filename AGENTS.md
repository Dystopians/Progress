...
**现状**：现行 UI 仍使用代码绘制图形，无随包字体。`assets/buildings/b01/` 已有 9 张跨时代建筑 PNG 候选（1254 × 1254、透明），全部待用户初审、未接入 UI；清单见 `assets/ASSETS.md`，审核见 `docs/_drafts/asset_review/b01/overview.html`。
...
### 2026-09-22 · Claude · 进行中 · 四百年重构 M1（总计划 docs/53）
...
- **Git**：仓库已有提交。基线 `d883734` 已推送；里程碑内部只做本地提交，里程碑结束时推送。`.gitattributes` 固定 LF（内容指纹依赖字节）。本地提交用 `git add -A`，会把 Codex 已落盘的 `assets/` 文件一并纳入版本库——只是存档，不代表接入或初审通过。
...
- **下一步**：M1-2 地区维度参数化 → M1-3 长期价格 → M1-4 容量回收 → M1-5 建筑堆 → M1-6/7 政府拆分与危机 → M1-8 批量推进 → M1-9 长局基准。
